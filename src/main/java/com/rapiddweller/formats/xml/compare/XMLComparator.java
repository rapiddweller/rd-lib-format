...
import static com.rapiddweller.formats.xml.compare.XMLComparisonModel.ATTRIBUTE;
import static com.rapiddweller.formats.xml.compare.XMLComparisonModel.DOCUMENT_ENCODING;
import static com.rapiddweller.formats.xml.compare.XMLComparisonModel.ELEMENT_NAME;
import static com.rapiddweller.formats.xml.compare.XMLComparisonModel.ELEMENT_NAMESPACE;
import static com.rapiddweller.formats.xml.compare.XMLComparisonModel.ELEMENT_TEXT;
import static com.rapiddweller.formats.xml.compare.XMLComparisonModel.PROCESSING_INSTRUCTION;
...
import com.rapiddweller.commons.converter.XMLNode2StringConverter;
import com.rapiddweller.commons.xml.XMLUtil;
import com.rapiddweller.formats.compare.AggregateDiff;
import com.rapiddweller.formats.compare.ArrayComparator;
import com.rapiddweller.formats.compare.ArrayComparisonResult;
import com.rapiddweller.formats.compare.DiffDetail;
import com.rapiddweller.formats.compare.DiffDetailType;
import com.rapiddweller.formats.compare.DiffFactory;
...
public class XMLComparator {
	...
	private void compareNodeArrays(Node[] expectedNodes, Node[] actualNodes,
		...
		ArrayComparisonResult result = ArrayComparator.compare(expectedNodes, actualNodes, settings.getModel(), parentPathOfExpected, parentPathOfActual, diffFactory);
		for (DiffDetail diff : result.getDiffs()) {
			if (diff.getType() == DiffDetailType.DIFFERENT && diff.getExpected() instanceof Element && diff.getActual() instanceof Element) {
				...
				compareElements((Element) diff.getExpected(), (Element) diff.getActual(), context, String.valueOf(diff.getLocatorOfExpected()), String.valueOf(diff.getLocatorOfActual()), diffs);
